...
def addtask():
    ...
    if form.validate_on_submit():
        ...
        new_task = Task(id=current_user.id, title=title, description=description, status=status , date=date)
...
def summary():
    ...
    count_todo =Task.query.filter_by(status='todo',id=current_user.id).all()
    count_doing = Task.query.filter_by(status='doing',id=current_user.id).all()
    count_done = Task.query.filter_by(status='done',id=current_user.id).all()

    return render_template('summary.html',todo= len(count_todo) ,doing=len(count_doing) , done = len(count_done))
@app.route('/logout')
@login_required
def logout():
    logout_user()
